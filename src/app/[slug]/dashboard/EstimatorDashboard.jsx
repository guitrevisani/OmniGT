...
function ifLabel(wkg, ftpW, cfg) {
  const IF = wkg / (ftpW / cfg.mass_kg);
  if (IF <= 0.70) return { label: "REGENERATIVO", color: "#4db6ac" };
  if (IF <= 0.81) return { label: "ENDURANCE",    color: "#8bc34a" };
  if (IF <= 0.95) return { label: "FORTE/LIMIAR", color: "#ffc107" };
  if (IF <= 1.04) return { label: "MÁXIMO",       color: "#ff1744" };
    ...
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:20 }}>
        <div style={{ flex:1, position:"relative" }}>
          ...
          <input type="range" min="0.5" max="1.1" step="0.05"
            value={(value / (ftpW / cfg.mass_kg)).toFixed(2)}
            onChange={e => onChange(parseFloat(e.target.value) * (ftpW / cfg.mass_kg))} />
...
export default function EstimatorDashboard({ slug, eventName }) {
  ...
  const exportImage = useCallback(() => {
    ...
    ctx.fillText("OGT · OMNI GT · treine.com.gt", W - PAD, H - 12);
