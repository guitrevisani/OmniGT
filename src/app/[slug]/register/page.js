...
export default function EventRegistrationPage() {
  ...
  const warn         = searchParams.get("warn") // "no_goals" vindo do callback

  const [event,             setEvent]             = useState(null)
  const [consentGiven,      setConsentGiven]      = useState(false)
  const [pushConsent,       setPushConsent]        = useState(false)
  const [keepGoals,         setKeepGoals]         = useState(true)  // marcado por padrão
  ...
  // Quando warn=no_goals, desmarca keepGoals automaticamente
  // para forçar o atleta a preencher os campos
  useEffect(() => {
  ...
  // Validação: se keepGoals=false, campos de meta são obrigatórios
  ...
  return (
    <div style={{ maxWidth: 560, margin: "2rem auto", fontFamily: "sans-serif", padding: "0 1rem" }}>
      ...
      {/* ── Campos de meta — desabilitados se keepGoals ─── */}
      <div style={{
      ...
      {/* ── Opt-in push — opcional, desmarcado por padrão ─ */}
